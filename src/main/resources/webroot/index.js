...
function newBoard() {
  return $.post("newBoard", JSON.stringify({
    ...
    message("Drag an animal to the board to start...", "success");
    updateBoard();
  });
}
...
function newPlayer(whichPlayer, name) {
  return $.post("newPlayer", JSON.stringify({
    ...
    if (name === "White") {
      imagesByID[player.id] = "White.png";
    } else if (name === "Black") {
      imagesByID[player.id] = "Black.png";
    }
  });
}
...
$(function () {
   ...
   */
  newBoard()
    .then(newPlayer("firstPlayer", "White"))
    .then(newPlayer("secondPlayer", "Black"));
});
